#!/usr/bin/env python3
"""Verify a binary-only ``age-and-gender`` install offline.

Run this against an installed wheel rather than the repository's ``src/``
tree. The checkout only supplies known-good images, compatibility fixtures,
and the bundled shape predictor. Network access is not required.
"""
...
def check_neural_dat_rejected(predictor) -> None:
    # Rejection happens before the file is read, so a placeholder is enough.
    with tempfile.TemporaryDirectory() as directory:
        ...
        try:
            ...
            print("neural .dat loader correctly rejected the dlib weight file")
            return
    raise SystemExit("load_dnn_age_predictor accepted a neural .dat file; it must not")
...
def check_compatibility_corpus(predictor) -> int:
    ...
    for case in manifest["inputs"]:
        ...
        if not path.is_file():
            raise SystemExit(f"compatibility-corpus image is missing: {case['path']}")
...
def main() -> None:
    ...
    # Mutates the shared predictor; keep it last so prior checks use the bundle.
